...
!--------------------------------------------------------------------!
      use Com_var, only : MAXn, epsa, epsr
      implicit none
      integer :: m, n
      real(8) :: S, S0, DS
      real(8), external :: f
...
!--------------------------------------------------------------------!
      ...
      parameter(f200='("|",3x,a,i3,12x," | ",23x,"|")')
      ...
      write(7,FM2)' ; w('//trim(adjustl(c))//') =',wj
          S = S + wj * f(x(j),m)
      end do
