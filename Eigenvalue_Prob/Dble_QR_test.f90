...
!====================================================================!
 ...
 AA:  do while(m > 0)
         ...
         call  Householder_1(N, m, a, P, plus, prod, Iter)
         do j=2, m-1
            call  Householder_j(N, m, j, a, P, Iter)
         end do
...
!====================================================================!
      subroutine  Householder_1(N, m, a, P, plus, prod, Iter)
      implicit none
...
!       write(6,*) 'r2 =',r2
      ...
      if (r2 /= 0.0d0) then
         r  = sign(sqrt(r2), b1)
         if (r2 + b1 * r == 0.0d0) stop '0 divide c2'
         c2 = 1.0d0 / (r2 + b1 * r)
         w(1) = b1 + r
         w(2) = b2 
         w(3) = b3 
...
!====================================================================!
      subroutine  Householder_j(N, m, j, a, P, Iter)
      implicit none
      integer, intent(in) :: N, m, j, Iter
      integer :: i, L, S, k
      real(8), intent(inout) :: a(N,N), P(N,N)
      real(8) :: q(N), qs(N), r2, c2, r, w(j:j+2), cpw
      real(8) :: underf=1.0d-300

      L = min(j+2, m)
      S = min(j+3, m)
      r2 = Dot_product(a(j:L,j-1), a(j:L,j-1))
          
      if (r2 /= 0.0d0) then
         r = sign(sqrt(Dot_product(a(j:L,j-1), a(j:L,j-1))), a(j,j-1))
         if (r2 + a(j,j-1) * r == 0.0d0)  return
         ...
         q(j:L) = q(j:L) - 0.5d0 * c2 * w(j:L) * Dot_product(q(j:L), w(j:L))
    !-------------
!        write(6,*) 'm =',m
!        write(6,*) 'j =',j
!        write(6,*) 'j: c2   =',c2
!        write(6,*) 'j: r    =',r
!        write(6,*) 'j: r2   =',r2
!        write(6,*) 'j: a(j) =',a(j,j-1)
!        do i=j, L
!           write(6,*) 'w(i)*c2 =',w(i) * c2
!        end do
!        do i=1, L
!           write(6,*) 'q(i) =',q(i)
!        end do
    !-------------
         ...
         qs(j:L) = qs(j:L) - 0.5d0 * c2 * w(j:L) * Dot_product(qs(j:L), w(j:L)) 
